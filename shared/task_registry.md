...
- `cabinet_id` — кабинет, в контексте которого идет работа по задаче;
...
Модульный агент:
1. Смотрит свои активные задачи через `./task mine --agent <module_name> [--cabinet-id <cabinet_id>]`.
...
4. В handoff обязательно пишет:
   - что сделано;
   - какой `cabinet_id` проверять;
   - где это проверять;
   - что именно reviewer должен проверить;
   - URL, файлы, порты и артефакты.

Reviewer:
1. По команде `task` смотрит свои непроверенные handoff через `./task mine --agent <reviewer_name> [--cabinet-id <cabinet_id>]`.
...
- `./task ensure-db`
- `./task mine --agent <agent_name> --cabinet-id <cabinet_id> --limit 20`
- `./task show --task-id <task_id> [--cabinet-id <cabinet_id>]`
- `./task list --task-id <task_id> [--cabinet-id <cabinet_id>] --limit 20`
- `./task add --task-id <task_id> --cabinet-id <cabinet_id> --source-agent <agent> --target-agent <agent> --module-name <module_name> --action-type start --summary "<что начато>" --status in_progress --artifacts "<paths>"`
- `./task add --task-id <task_id> --cabinet-id <cabinet_id> --source-agent <module_agent> --target-agent <reviewer_agent> --module-name <module_name> --action-type handoff --status pending --summary "<что сделано>" --implementation-report "<где и как проверять>" --checks-required "<что reviewer обязан проверить>" --artifacts "<paths_or_urls>"`
...
- `./task add --task-id <task_id> --cabinet-id <cabinet_id> --source-agent <module_agent> --target-agent <module_agent> --module-name <module_name> --action-type done --status done --summary "<что завершено>" --artifacts "<paths>"`
- `./task sync`
...
- handoff, review и event должны ссылаться на тот же `cabinet_id`, что и сама задача;
